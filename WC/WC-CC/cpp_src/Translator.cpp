...
int main ()
{
	...
	double sent_time;
	ifstream inf1;
	inf1.open("WC_CC_TSN.txt");
	ifstream inf2;
	inf2.open("WC_A_TSN.txt");
	ifstream inf3;
	inf3.open("WC_M_TSN.txt");
	ofstream outf1;
	outf1.open("WC_CC_5G.txt");
	ofstream outf2;
	outf2.open("WC_A_5G.txt");
	ofstream outf3;
	outf3.open("WC_M_5G.txt");
	...
	{	
		...
		{							
			...
			if(payload<256)
			{
				outf1<<fixed<<setprecision(9)<<sent_time+x<<' '<<payload+2<<' '<<v[2]<<" (R=0 F=0 LCID=000101 L="<<payload<<") TCP_SEQ="<<v[3]<<" TCP_ACK="<<v[4]<<' '<<v[5]<<endl;
			}
			else
			{
				outf1<<fixed<<setprecision(9)<<sent_time+x<<' '<<payload+3<<' '<<v[2]<<" (R=0 F=1 LCID=000101 L="<<payload<<") TCP_SEQ="<<v[3]<<" TCP_ACK="<<v[4]<<' '<<v[5]<<endl;
			}					
		}*/
		PCP5++;
	...
	cout << "Start 5G/TSN Translation for PCP0 (S.D Uploading)"<<endl;
	usleep(3000000);
	while(getline(inf3, s))  //  將文檔一行一行讀入字串s
	{	
		SplitString(s, v, " ");
		if(v[2]=="PCP=0"&&v.size()==4)			
		{		
			...
			if(payload<256)
			{
				outf3<<fixed<<setprecision(9)<<sent_time+x<<' '<<payload+2<<' '<<v[2]<<" (R=0 F=0 LCID=000000 L="<<payload<<") "<<v[3]<<endl;
			}
			else
			{
				outf3<<fixed<<setprecision(9)<<sent_time+x<<' '<<payload+3<<' '<<v[2]<<" (R=0 F=1 LCID=000000 L="<<payload<<") "<<v[3]<<endl;
			}
			
		}
		/*else if(v[2]=="PCP=1"&&v.size()==4)			
		{							
			stringstream ss(v[1].c_str());
			ss>>payload; // 數字字串轉成int
			stringstream sst(v[0].c_str());
			...
			if(payload<256)
			{
				outf3<<fixed<<setprecision(9)<<sent_time+x<<' '<<payload+2<<' '<<v[2]<<" (R=0 F=0 LCID=000001 L="<<payload<<") "<<v[3]<<endl;
			}
			else
			{
				outf3<<fixed<<setprecision(9)<<sent_time+x<<' '<<payload+3<<' '<<v[2]<<" (R=0 F=1 LCID=000001 L="<<payload<<") "<<v[3]<<endl;
			}
			
		}*/
		PCP0++;
		
		cout << "[" << PCP0 <<"]" << "Adding 5G header time:" << x <<endl;
