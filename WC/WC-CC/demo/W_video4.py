...
video3 = cv2.VideoCapture("/home/lukelin/Desktop/WC-CC/demo/WW.mp4")
...
input_1 = '/home/lukelin/Desktop/result/output/WGCL_Att.txt'
...
for i in range(len(x)):
	...
	if i%ratio==0:
		...
		cv2.namedWindow("[WGCL] Warning", cv2.WINDOW_NORMAL)
		cv2.imshow("[WGCL] Warning", frame2)
		cv2.moveWindow("[WGCL] Warning",650,370)
		cv2.resizeWindow("[WGCL] Warning",600,300)
		
		cv2.namedWindow("[WGCL] Transmission Time", cv2.WINDOW_NORMAL)
		cv2.imshow("[WGCL] Transmission Time", frame3)
		cv2.moveWindow("[WGCL] Transmission Time",1250,370)
		cv2.resizeWindow("[WGCL] Transmission Time",600,300)
		if y[i] < 0.03:
			waitpara = 1
		else:
			waitpara = int(y[i]/0.03)
...
if float(error_count/i) < 0.001:
	cv2.imshow("[WGCL] Warning", fine)
else:
	cv2.imshow("[WGCL] Warning", bad)
cv2.moveWindow("[WGCL] Warning",650,370)
cv2.resizeWindow("[WGCL] Warning",600,300)
cv2.waitKey(0)							
