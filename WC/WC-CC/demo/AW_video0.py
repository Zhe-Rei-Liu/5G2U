...
video3 = cv2.VideoCapture("/home/lukelin/Desktop/WC-CC/demo/AWS.mp4")
...
input_1 = '/home/lukelin/Desktop/result/output/AW-WGCL_Mtt.txt'
...
for i in range(len(x)):
	...
	if i%ratio==0:
		...
		cv2.namedWindow("[AW-WGCL] S.D Uploading", cv2.WINDOW_NORMAL)
		cv2.imshow("[AW-WGCL] S.D Uploading", frame2)	
		cv2.moveWindow("[AW-WGCL] S.D Uploading",650,720)
		cv2.resizeWindow("[AW-WGCL] S.D Uploading",600,300)
		
		cv2.namedWindow("[AW-WGCL] Transmission Time", cv2.WINDOW_NORMAL)
		cv2.imshow("[AW-WGCL] Transmission Time", frame3)
		cv2.moveWindow("[AW-WGCL] Transmission Time",1250,720)
...
if float(error_count/i) < 0.1:
	cv2.imshow("[AW-WGCL] S.D Uploading", fine)
else:
	cv2.imshow("[AW-WGCL] S.D Uploading", bad)
cv2.moveWindow("[AW-WGCL] S.D Uploading",650,720)
cv2.resizeWindow("[AW-WGCL] S.D Uploading",600,300)
cv2.waitKey(0)							
