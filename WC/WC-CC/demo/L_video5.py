...
video3 = cv2.VideoCapture("/home/lukelin/Desktop/WC-CC/demo/LCC.mp4")
...
input_1 = '/home/lukelin/Desktop/result/output/LCP_CCtt.txt'
...
for i in range(len(x)):
	...
	if i%ratio==0:
		...
		cv2.namedWindow("[LCP] Control Command", cv2.WINDOW_NORMAL)
		cv2.imshow("[LCP] Control Command", frame2)
		cv2.moveWindow("[LCP] Control Command",650,1)
		cv2.resizeWindow("[LCP] Control Command",600,300)
		
		cv2.namedWindow("[LCP] Transmission Time", cv2.WINDOW_NORMAL)
		cv2.imshow("[LCP] Transmission Time", frame3)
		cv2.moveWindow("[LCP] Transmission Time",1250,1)
...
if float(error_count/i) < 0.001:
	cv2.imshow("[LCP] Control Command", fine)
else:
	cv2.imshow("[LCP] Control Command", bad)
cv2.moveWindow("[LCP] Control Command",650,1)
cv2.resizeWindow("[LCP] Control Command",600,300)
cv2.waitKey(0)			
