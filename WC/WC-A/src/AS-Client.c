...
{	
    if (argc != 3) 
    {
        ...
        printf("[Try type]./AS-Client 100 2000\n");
        return 1;
    }
    ...
    UA_StatusCode retval = UA_Client_connect(client, "opc.tcp://192.168.0.107:4841");
	...
	{	
		...
		if(count >= msg_num)
		{
			UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_CLIENT, "%d alarm are sent!!", count);
			loop_switch = 0;
		}
	}
