...
file = '/home/lukelin/Desktop/result/output/RR_CCWT_1.txt' 
W_ccwt_1 = []
with open(file,'r') as f:
	lines = f.readlines()
	for line in lines:
...
file = '/home/lukelin/Desktop/result/output/RR_AWT_1.txt' 
W_awt_1 = []
with open(file,'r') as f:
	lines = f.readlines()
	for line in lines:
...
file = '/home/lukelin/Desktop/result/output/RR_MWT.txt' 
W_mwt = []
with open(file,'r') as f:
	lines = f.readlines()
	for line in lines:
		...
		mnumber = mnumber +1
print("\nPercentage of Bandwidth Distributed by Round Robin:")
...
p2 = plt.plot(x, W_ccwt_1,linestyle = '-',color = 'red' ,label = 'RR')
...
plt.text(91, 0.5 , 'RR: %0.3f%%'%(atotalWT_2/90500.0*100.0),fontsize=10)
plt.text(91, 0.4 , '----------------------------',fontsize=10)
plt.legend(bbox_to_anchor=(1.09, 1.05))

plt.subplot(3,1,3)
p1 = plt.plot(x, AW_mwt,linestyle = '-',color = 'blue' ,label = 'AW-WGCL')
p2 = plt.plot(x, W_mwt,linestyle = '-',color = 'red' ,label = 'RR')
x_major_locator = MultipleLocator(5)
ax = plt.gca()
ax.xaxis.set_major_locator(x_major_locator)
plt.xlim(0,91)
plt.ylim(0,1)
plt.title('Window Time Comparison of S.D Uploading')
plt.xlabel('Time')
plt.ylabel('Window Time per TTI (ms)')
plt.text(91, 0.7 , '--------Avg. BW--------',fontsize=10)
plt.text(91, 0.6 , 'AW-WGCL: %0.3f%%'%(mtotalWT_1/90500.0*100.0),fontsize=10)
plt.text(91, 0.5 , 'RR: %0.3f%%'%(mtotalWT_2/90500.0*100.0),fontsize=10)
plt.text(91, 0.4 , '----------------------------',fontsize=10)
plt.legend(bbox_to_anchor=(1.09, 1.05))
