...
plt.plot(x1,y, marker = '.',linestyle = '-',label = 'Control Command Execution Latency')
#bar1 = plt.bar(x, y, color='blue')
plt.text(1250, .0095, r'$\mu=%0.9f$'%(mean)+'   $\sigma=%0.9f$'%(st_dev)+'    Max=%0.9f'%(maxval))
plt.plot(x1,mean_list, linestyle = '--',color = 'black', label = 'Avg. Control Command Execution Latency')
plt.xlim(0,2020)
plt.ylim(0,0.01)
